...
import { ApplicationCommandCategory, HighlightStyle, type ApplicationCommand, type Interaction } from 'types/types';
import { emoji, highlight, hyperlink } from 'utils/markdown';
import { redis } from 'utils/redis';
import or, { toEmoji } from 'utils/utils';
          ...
          components: [
            ...
            },
            {
              ...
              components: [
                {
                  type: MessageComponentTypes.TextDisplay,
                  content: `You can view all the available commands by typing ${highlight('/', HighlightStyle.Bold)}, or by clicking the button on the right, for further game assistance which the bot may or may not display go to ${hyperlink('https://forgewiki.org/wiki/The_Forge_wiki', 'the official wiki')}.`,
                },
              ],
              accessory: {
            ...
            {
              type: MessageComponentTypes.TextDisplay,
              content: `You can support the bot by voting it in **${hyperlink('https://top.gg/bot/1461873695688491190/vote', 'top.gg')}**! Every vote helps is appreciated, helps the bot grow and reach more people.`,
            },
            {
              type: MessageComponentTypes.TextDisplay,
      ...
      if (i.data.customId === 'view-all-commands') {
        ...
        const commandList = [...bot.commands.values()]
          ...
          .map((cmd) => {
            ...
            return id ? `> </${cmd.name}:${id}> - ${cmd.description}` : `> \`/${cmd.name}\` - ${cmd.description}`;
          })
          .join('\n');
            ...
            components: [
              {
                ...
                components: [
                  {
                    type: MessageComponentTypes.TextDisplay,
                    content: `${emoji('Warning')} Please provide a valid image file.`,
                  },
                ],
              },
            ],
          ...
          components: [
            {
              ...
              components: [
                {
                  type: MessageComponentTypes.TextDisplay,
                  content: `${emoji('Correct')} Bug report submitted successfully!\n-# Your bug report ID: ${bugReportId}`,
                },
              ],
            },
          ],
